"""
Quick Electricity (ENTSO-E) evaluation using the working API
"""
...
def evaluate_model(pipeline, test_samples, device='mps'):
    """Evaluate model using pipeline.predict()"""
    ...
    for i, (context, target) in enumerate(test_samples):
        try:
            ...
            median_idx = 4
            forecast_median = fc_tensor[:, median_idx, :]
...
# Load data
print("="*70)
print("ELECTRICITY (ENTSO-E) EVALUATION")
print("="*70)
...
# Save
with open('./results/electricity_evaluation.json', 'w') as f:
...
print(f"\n✓ Results saved to: ./results/electricity_evaluation.json")
